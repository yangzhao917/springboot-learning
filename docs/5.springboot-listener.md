...
sequenceDiagram
    ...
    Publisher->>Container: 2. 接收事件对象
    activate Container
    
    Container->>Container: 3. 查找匹配的监听器
    
    Container->>Container: 4. 按Order注解排序
    
    Container->>Listener1: 5. 执行监听器1<br/>(同步)
    activate Listener1
    Listener1->>Listener1: 处理事件逻辑
    Listener1-->>Container: 执行完成
    deactivate Listener1
    
    Container->>Listener2: 6. 执行监听器2<br/>(同步)
    activate Listener2
    Listener2->>Listener2: 处理事件逻辑
    Listener2-->>Container: 执行完成
    deactivate Listener2
    
    Container-->>Publisher: 7. 所有监听器执行完成
    deactivate Container
    
    Publisher-->>Service: 8. 返回控制权
