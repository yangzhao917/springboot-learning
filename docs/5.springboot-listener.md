...
sequenceDiagram
    ...
    participant Registry as 监听器注册表
    participant Listener1 as 监听器1(@Order(1))
    participant Listener2 as 监听器2(@Order(2))
    participant Listener3 as 异步监听器3(@Async)

    Service->>Publisher: 1. publishEvent(event)
    Publisher->>Container: 2. 发布事件到容器
    Container->>Registry: 3. 查找匹配的监听器
    Registry-->>Container: 4. 返回监听器列表
    
    Note over Container,Listener2: 同步监听器按顺序执行
    
    Container->>Listener1: 5. 执行监听器1
    activate Listener1
    Listener1->>Listener1: 处理事件逻辑
    Listener1-->>Container: 执行完成
    deactivate Listener1
    
    Container->>Listener2: 6. 执行监听器2
    activate Listener2
    Listener2->>Listener2: 处理事件逻辑
    Listener2-->>Container: 执行完成
    deactivate Listener2
    
    Note over Listener3: 异步监听器在独立线程执行
    
    Container->>Listener3: 7. 提交异步监听器到线程池
    activate Listener3
    Listener3->>Listener3: 异步处理事件逻辑
    Listener3-->>Container: 异步执行完成
    deactivate Listener3
    
    Container-->>Publisher: 8. 所有监听器处理完成
    Publisher-->>Service: 9. 返回控制权
```
...
```mermaid
flowchart TD
    Start([开始: 业务逻辑触发]) --> Publish[发布事件<br/>publishEvent]
    Publish --> Container{Spring容器<br/>接收事件}
    
    Container --> Match[查找匹配的监听器]
    Match --> Filter{过滤监听器}
    
    Filter -->|检查条件| CheckCondition{条件是否满足?}
    CheckCondition -->|否| Skip[跳过该监听器]
    CheckCondition -->|是| AddToList[添加到执行列表]
    
    Filter --> Sort[按@Order排序]
    Sort --> Split{分离同步和异步}
    
    Split --> SyncList[同步监听器列表]
    Split --> AsyncList[异步监听器列表]
    
    SyncList --> ExecuteSync[顺序执行同步监听器]
    ExecuteSync --> Loop{还有同步监听器?}
    Loop -->|是| ExecListener[执行监听器方法]
    ExecListener --> HandleException{是否异常?}
    HandleException -->|是| LogError[记录异常<br/>继续执行下一个]
    HandleException -->|否| Loop
    LogError --> Loop
    Loop -->|否| AsyncList
    
    AsyncList --> SubmitAsync[提交异步监听器到线程池]
    SubmitAsync --> AsyncExec[异步执行监听器]
    AsyncExec --> Return([返回控制权给业务服务])
...
```mermaid
flowchart LR
    Start([应用启动]) --> Scan[扫描组件]
    Scan --> FindListener{发现监听器}
    
    FindListener -->|@Component + @EventListener| Annotated[注解监听器]
    FindListener -->|实现ApplicationListener| Interface[接口监听器]
    FindListener -->|实现SmartApplicationListener| Smart[智能监听器]
    FindListener -->|@Bean方法返回| BeanMethod[Bean方法监听器]
    
    Annotated --> Parse[解析监听器信息]
    Interface --> Parse
    Smart --> Parse
    BeanMethod --> Parse
    
    Parse --> Extract[提取事件类型]
    Extract --> Register[注册到监听器注册表]
    
    Register --> Store[存储到<br/>ApplicationListenerRegistry]
    Store --> Ready([监听器就绪])
...
flowchart TD
    subgraph 同步执行模式
        Start1([业务服务调用]) --> Pub1[发布事件]
        Pub1 --> SyncExec[同步执行监听器]
        SyncExec --> Wait[等待所有监听器执行完成]
        Wait --> Return1([返回控制权])
        Return1 --> Continue1[继续业务逻辑]
    end
    
    subgraph 异步执行模式
        Start2([业务服务调用]) --> Pub2[发布事件]
        Pub2 --> AsyncExec[提交异步监听器]
        AsyncExec --> Return2([立即返回控制权])
        Return2 --> Continue2[继续业务逻辑]
        AsyncExec --> ThreadPool[线程池执行]
        ThreadPool --> AsyncProcess[异步处理事件]
    end
...
```mermaid
sequenceDiagram
    participant Service as 业务服务(@Transactional)
    participant Transaction as 事务管理器
    participant Publisher as ApplicationEventPublisher
    participant EventQueue as 事务事件队列
    participant Listener as 事务监听器

    Service->>Transaction: 1. 开始事务
    activate Transaction
    
    Service->>Service: 2. 执行业务逻辑
    
    Service->>Publisher: 3. publishEvent(event)
    Publisher->>EventQueue: 4. 将事件加入队列<br/>(不立即执行)
    EventQueue-->>Publisher: 事件已入队
    Publisher-->>Service: 返回(非阻塞)
    
    Service->>Service: 5. 继续业务逻辑
    
    Service->>Transaction: 6. 提交事务
    Transaction->>EventQueue: 7. 事务提交成功<br/>触发事件处理
    
    EventQueue->>Listener: 8. 执行监听器<br/>(AFTER_COMMIT阶段)
    activate Listener
    Listener->>Listener: 处理事件逻辑
    Listener-->>EventQueue: 执行完成
    deactivate Listener
    
    Transaction-->>Service: 9. 事务提交完成
    
    Note over Transaction: 如果事务回滚
    Transaction->>EventQueue: 7b. 事务回滚<br/>触发AFTER_ROLLBACK监听器
```
...
- [6. Spring Boot AOP](6.springboot-aop.md)：学习面向切面编程
- [7. Spring Boot Security](7.springboot-security.md)：学习安全认证与授权
- [8. Spring Boot Cache](8.springboot-cache.md)：学习缓存机制
