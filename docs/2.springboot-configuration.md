# Spring Boot 配置

> 摘要（TL;DR）
> - 目标：快速掌握 Properties/YAML、Profile 与加载顺序
> - 学会：三种读取方式（@Value/Environment/@ConfigurationProperties）与最佳实践
> - 收获：会组织配置、按环境切换、定位配置冲突与覆盖

## 📚 目录

- [内容概览](#内容概览)
- [2.1 什么是Spring Boot配置](#21-什么是spring-boot配置)
- [2.2 配置文件格式与属性引用](#22-配置文件格式与属性引用)
- [2.3 约定优于配置（Convention over Configuration）](#23-约定优于配置convention-over-configuration)
- [2.4 配置文件加载顺序](#24-配置文件加载顺序)
- [2.5 环境配置（Profile）](#25-环境配置profile)
- [2.6 常用配置项](#26-常用配置项)
- [2.7 配置读取方式](#27-配置读取方式)
- [2.8 复杂数据格式与特殊字符处理](#28-复杂数据格式与特殊字符处理)
- [2.9 最佳实践](#29-最佳实践)
- [2.10 总结](#210-总结)

---

## 内容概览

本文档全面介绍Spring Boot配置系统，涵盖以下核心内容：

### 📋 基础概念
- **配置定义**：Spring Boot配置是告诉应用程序如何运行的方式
- **约定优于配置**：Spring Boot的核心思想，提供合理的默认值，减少配置工作

### 📝 配置文件格式
- **Properties格式**：简单直观的key=value格式，适合简单配置
- **YAML格式**：结构清晰的缩进格式，适合复杂配置
- **属性引用**：支持配置值之间的引用和占位符 `${property.name:defaultValue}`

### 🔧 配置读取方式
- **@Value注解**：读取单个配置值，简单直接，性能好
- **Environment接口**：动态读取配置，支持运行时条件判断
- **@ConfigurationProperties**：复杂配置结构，类型安全，支持验证

### 🏗️ 高级特性
- **复杂数据结构**：支持数组、Map、嵌套对象等复杂数据格式
- **特殊字符处理**：正确处理引号、转义字符、Unicode、多行文本等
- **配置验证**：使用Bean Validation确保配置正确性
- **环境管理**：使用Profile分离不同环境配置

### 📚 最佳实践
- **配置组织**：环境分离、敏感信息保护、配置分层管理
- **格式选择**：简单配置用Properties，复杂配置用YAML
- **属性引用**：合理使用占位符和默认值，避免循环引用

---

## 2.1 什么是Spring Boot配置

Spring Boot配置是告诉应用程序如何运行的方式。就像给手机设置铃声、壁纸一样，我们需要告诉Spring Boot应用：
- 在哪个端口运行（比如8080端口）
- 连接哪个数据库
- 日志记录到什么级别
- 等等...

## 2.2 配置文件格式与属性引用

Spring Boot支持两种主要的配置文件格式：Properties和YAML，并且都支持属性引用和占位符功能。

### 2.2.1 Properties格式 (.properties)
```properties
# 这是注释，以#开头
server.port=8080
server.servlet.context-path=/myapp
spring.application.name=my-spring-app

# 字符串配置
app.title=我的Spring Boot应用
app.description=这是一个示例应用

# 数字配置
app.timeout=30
app.retry-count=3

# 布尔值配置
app.enabled=true
app.debug=false

# 数组/列表配置（使用逗号分隔）
app.servers=server1,server2,server3
app.ports=8080,8081,8082

# Map配置（使用点号表示层级关系）
app.database.host=localhost
app.database.port=3306
app.database.name=mydb
app.database.username=root
app.database.password=123456

# 嵌套对象配置
app.redis.host=localhost
app.redis.port=6379
app.redis.timeout=2000
app.redis.pool.max-active=8
app.redis.pool.max-idle=8
app.redis.pool.min-idle=0

# 多值Map配置
app.users.admin.name=管理员
app.users.admin.role=ADMIN
app.users.user.name=普通用户
app.users.user.role=USER

# 属性引用和占位符
# 1. 引用其他配置值
app.welcome.message=欢迎使用${app.title}！
app.database.url=jdbc:mysql://${app.database.host}:${app.database.port}/${app.database.name}

# 2. 使用默认值
app.timeout=${app.retry-count:5}
app.debug=${app.enabled:false}

# 3. 引用系统属性
app.user.home=${user.home}
app.java.version=${java.version}

# 4. 引用环境变量
app.database.password=${DB_PASSWORD:defaultPassword}
app.redis.host=${REDIS_HOST:localhost}

# 5. 嵌套属性引用
app.connection.url=${app.database.url}?useSSL=${app.database.ssl:false}
app.logging.level=${app.debug:false:DEBUG:INFO}

# 6. 复杂属性引用
app.full.title=${app.title} - ${spring.application.name}
app.api.base-url=http://${server.address:localhost}:${server.port}${server.servlet.context-path}
```

**Properties格式特点：**
- 使用 `key=value` 的格式
- 注释用 `#` 开头
- 简单直观，适合简单的配置
- 支持嵌套结构，使用点号分隔层级
- 数组使用逗号分隔多个值
- 支持属性引用和占位符 `${property.name:defaultValue}`

### 2.2.2 YAML格式 (.yml 或 .yaml)
```yaml
# 这是注释，以#开头
server:
  port: 8083
  servlet:
    context-path: /myapp
spring:
  application:
    name: my-spring-app

# 字符串配置
app:
  title: 我的Spring Boot应用
  description: 这是一个示例应用

# 数字配置
  timeout: 30
  retry-count: 3

# 布尔值配置
  enabled: true
  debug: false

# 数组/列表配置
  servers:
    - server1
    - server2
    - server3
  ports: [8080, 8081, 8082]  # 行内数组格式

# Map配置
  database:
    host: localhost
    port: 3306
    name: mydb
    username: root
    password: 123456

# 嵌套对象配置
  redis:
    host: localhost
    port: 6379
    timeout: 2000
    pool:
      max-active: 8
      max-idle: 8
      min-idle: 0

# 复杂数据结构示例
  users:
    admin:
      name: 管理员
      role: ADMIN
      permissions:
        - READ
        - WRITE
        - DELETE
    user:
      name: 普通用户
      role: USER
      permissions:
        - READ

# 多层级列表
  features:
    - name: 用户管理
      enabled: true
      config:
        max-users: 1000
        auto-approve: false
    - name: 数据分析
      enabled: false
      config:
        batch-size: 100
        schedule: "0 0 2 * * ?"

# 环境特定配置示例
  environments:
    dev:
      debug: true
      log-level: DEBUG
    test:
      debug: false
      log-level: INFO
    prod:
      debug: false
      log-level: WARN

# 属性引用和占位符
# 1. 引用其他配置值
  welcome:
    message: "欢迎使用${app.title}！"
  
  database:
    url: "jdbc:mysql://${app.database.host}:${app.database.port}/${app.database.name}"

# 2. 使用默认值
  timeout: ${app.retry-count:5}
  debug: ${app.enabled:false}

# 3. 引用系统属性
  user:
    home: ${user.home}
  java:
    version: ${java.version}

# 4. 引用环境变量
  database:
    password: ${DB_PASSWORD:defaultPassword}
  redis:
    host: ${REDIS_HOST:localhost}

# 5. 嵌套属性引用
  connection:
    url: "${app.database.url}?useSSL=${app.database.ssl:false}"
  logging:
    level: "${app.debug:false:DEBUG:INFO}"

# 6. 复杂属性引用
  full:
    title: "${app.title} - ${spring.application.name}"
  api:
    base-url: "http://${server.address:localhost}:${server.port}${server.servlet.context-path}"

# 7. 多行字符串中的属性引用
  description: |
    应用名称: ${spring.application.name}
    端口: ${server.port}
    上下文路径: ${server.servlet.context-path}
    数据库: ${app.database.host}:${app.database.port}

# 8. 条件属性引用
  features:
    user-management:
      enabled: ${app.features.user-management.enabled:true}
      max-users: ${app.features.user-management.max-users:1000}
    data-analysis:
      enabled: ${app.features.data-analysis.enabled:false}
      batch-size: ${app.features.data-analysis.batch-size:100}
```

**YAML格式特点：**
- 使用缩进表示层级关系
- 更易读，结构清晰
- 适合复杂的配置
- 支持多种数据类型：字符串、数字、布尔值、数组、对象
- 数组可以用 `-` 或 `[]` 表示
- 对象用缩进表示嵌套关系
- 支持属性引用和占位符 `${property.name:defaultValue}`

### 2.2.3 属性引用详解

Spring Boot支持在配置文件中使用占位符来引用其他配置值，语法为：`${property.name:defaultValue}`

**属性引用类型：**
1. **引用其他配置值**：`app.welcome.message=欢迎使用${app.title}！`
2. **使用默认值**：`app.timeout=${app.retry-count:30}`
3. **引用系统属性**：`app.user.home=${user.home}`
4. **引用环境变量**：`app.database.password=${DB_PASSWORD:defaultPassword}`
5. **嵌套属性引用**：`app.connection.url=${app.database.url}?useSSL=${app.database.ssl:false}`
6. **复杂属性引用**：`app.full.title=${app.title} - ${spring.application.name}`

**引用顺序：** 命令行参数 → 环境变量 → 配置文件 → 系统属性 → 默认值

**最佳实践：**
- 提供有意义的默认值
- 避免循环引用
- 使用环境变量覆盖敏感信息
- 保持配置层次结构清晰

## 2.3 约定优于配置（Convention over Configuration）

这是Spring Boot的核心思想：**"约定优于配置"**

### 2.3.1 约定优于配置是怎么来的？

#### 历史背景
在Spring Boot出现之前，Java企业级应用开发面临很多问题：

**传统Spring开发的问题：**
- 需要写大量的XML配置文件
- 配置复杂，容易出错
- 项目启动慢，开发效率低
- 新手学习成本高

**举个例子，传统Spring需要这样配置：**
```xml
<!-- 传统Spring需要大量XML配置 -->
<beans>
    <bean id="dataSource" class="org.springframework.jdbc.datasource.DriverManagerDataSource">
        <property name="driverClassName" value="com.mysql.jdbc.Driver"/>
        <property name="url" value="jdbc:mysql://localhost:3306/mydb"/>
        <property name="username" value="root"/>
        <property name="password" value="123456"/>
    </bean>
    
    <bean id="jdbcTemplate" class="org.springframework.jdbc.core.JdbcTemplate">
        <property name="dataSource" ref="dataSource"/>
    </bean>
    
    <!-- 还有更多配置... -->
</beans>
```

#### 设计理念的诞生
Spring Boot团队从Ruby on Rails框架中借鉴了"约定优于配置"的理念：

**核心思想：**
- 为常见场景提供**合理的默认值**
- 减少开发者的配置工作
- 让应用能够"开箱即用"
- 只有在需要时才覆盖默认行为

### 2.3.2 解决了什么问题？

#### 问题1：配置复杂
**之前：** 需要写大量XML配置
**现在：** 大部分配置都有默认值，零配置就能运行

#### 问题2：学习成本高
**之前：** 新手需要学习大量配置知识
**现在：** 按照约定就能快速上手

#### 问题3：项目启动慢
**之前：** 需要加载大量配置文件
**现在：** 按需加载，启动更快

#### 问题4：配置容易出错
**之前：** 手动配置容易写错
**现在：** 默认配置经过测试，更可靠

### 2.3.3 约定的配置哪里可以看？

#### 方法1：官方文档
Spring Boot官方文档提供了完整的配置参考：
- 官网：https://docs.spring.io/spring-boot/docs/current/reference/html/application-properties.html
- 中文版：https://springdoc.cn/spring-boot.html

#### 方法2：IDE自动补全
在IntelliJ IDEA或Eclipse中，输入配置时会有自动补全提示：

```properties
# 输入 spring. 会有自动补全
spring.datasource.url=
spring.datasource.username=
spring.datasource.password=
```

#### 方法3：源码查看
Spring Boot的默认配置定义在源码中：

```java
// 在Spring Boot源码中查找
@ConfigurationProperties(prefix = "server")
public class ServerProperties {
    private Integer port = 8080;  // 默认端口8080
    private String servletContextPath = "/";  // 默认上下文路径
    // ... 更多默认配置
}
```

#### 方法4：运行时查看
启动应用后，访问 `/actuator/configprops` 端点（需要添加actuator依赖）：

```json
{
  "server": {
    "port": 8080,
    "servlet": {
      "contextPath": "/"
    }
  }
}
```

#### 方法5：常用默认配置速查表

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `server.port` | 8080 | 服务器端口 |
| `server.servlet.context-path` | / | 应用上下文路径 |
| `spring.application.name` | 空 | 应用名称 |
| `logging.level.root` | INFO | 根日志级别 |
| `spring.datasource.driver-class-name` | 根据URL自动检测 | 数据库驱动 |
| `spring.jpa.hibernate.ddl-auto` | create-drop | JPA表创建策略 |

### 2.3.4 什么是约定优于配置？

Spring Boot会提供很多**默认值**，你不需要写配置，它就能正常工作。

**举个例子：**
- 默认端口：8080（你不需要写 `server.port=8080`）
- 默认上下文路径：/（你不需要写 `server.servlet.context-path=/`）
- 默认静态资源位置：`/static`、`/public`、`/resources`、`/META-INF/resources`

### 2.3.5 什么时候需要配置？

当你想要**改变默认行为**时：

```properties
# 改变默认端口
server.port=8081

# 改变默认上下文路径
server.servlet.context-path=/api

# 改变默认静态资源位置
spring.web.resources.static-locations=classpath:/custom-static/
```

### 2.3.6 约定优于配置的优势

#### 开发效率提升
- **快速启动**：新建项目几分钟就能运行
- **减少错误**：默认配置经过充分测试
- **专注业务**：不用花时间在配置上

#### 团队协作友好
- **统一标准**：团队都遵循相同的约定
- **易于维护**：配置少，维护成本低
- **知识传承**：新人容易上手

#### 最佳实践体现
- **经验积累**：约定基于Spring团队的最佳实践
- **持续优化**：默认配置会随着版本更新而优化
- **社区认可**：被广泛接受和使用的标准

## 2.4 配置文件加载顺序

Spring Boot会按以下顺序加载配置文件，**后面的会覆盖前面的**：

### 2.4.1 加载顺序（从低到高优先级）

1. **默认配置** - Spring Boot内置的默认值
2. **application.properties** - 项目根目录的配置文件
3. **application.yml** - 项目根目录的配置文件
4. **application-{profile}.properties** - 环境特定配置
5. **application-{profile}.yml** - 环境特定配置
6. **命令行参数** - 启动时通过 `--` 传递的参数
7. **环境变量** - 系统环境变量

### 2.4.2 实际例子

假设你有以下配置：

**application.properties:**
```properties
server.port=8080
spring.application.name=my-app
```

**application-dev.properties:**
```properties
server.port=8081
logging.level.org.springframework=DEBUG
```

**启动命令:**
```bash
java -jar app.jar --server.port=8082
```

**最终生效的配置:**
- `server.port=8082` (命令行参数优先级最高)
- `spring.application.name=my-app` (来自application.properties)
- `logging.level.org.springframework=DEBUG` (来自application-dev.properties)

## 2.5 环境配置（Profile）

### 2.5.1 什么是Profile？

Profile是Spring Boot用来区分不同环境的机制，比如：
- `dev` - 开发环境
- `test` - 测试环境  
- `prod` - 生产环境

### 2.5.2 如何使用Profile？

**方法1：在配置文件中指定**
```properties
# application.properties
spring.profiles.active=dev
```

**方法2：启动时指定**
```bash
java -jar app.jar --spring.profiles.active=prod
```

**方法3：环境变量**
```bash
export SPRING_PROFILES_ACTIVE=prod
java -jar app.jar
```

### 2.5.3 配置文件命名规则

- `application.properties` - 默认配置
- `application-dev.properties` - 开发环境配置
- `application-test.properties` - 测试环境配置
- `application-prod.properties` - 生产环境配置

## 2.6 常用配置项

### 2.6.1 服务器配置
```properties
# 端口号
server.port=8080

# 上下文路径
server.servlet.context-path=/myapp

# 服务器地址
server.address=0.0.0.0
```

### 2.6.2 应用配置
```properties
# 应用名称
spring.application.name=my-spring-app

# 应用版本
spring.application.version=1.0.0
```

### 2.6.3 数据库配置
```properties
# 数据库连接
spring.datasource.url=jdbc:mysql://localhost:3306/mydb
spring.datasource.username=root
spring.datasource.password=123456
spring.datasource.driver-class-name=com.mysql.cj.jdbc.Driver
```

### 2.6.4 日志配置
```properties
# 日志级别
logging.level.org.springframework=INFO
logging.level.com.example=DEBUG

# 日志文件
logging.file.name=logs/app.log
logging.pattern.console=%d{yyyy-MM-dd HH:mm:ss} - %msg%n
```

## 2.7 配置读取方式

Spring Boot提供了三种主要的配置读取方式，各有其适用场景：

### 2.7.1 @Value注解 - 简单配置读取

**适用场景：** 读取单个配置值，简单直接

**基本用法：**
```java
@RestController
public class ConfigController {
    
    @Value("${server.port:8080}")
    private String port;
    
    @Value("${spring.application.name:unknown}")
    private String appName;
    
    @GetMapping("/config")
    public String getConfig() {
        return "端口: " + port + ", 应用名: " + appName;
    }
}
```

**复杂配置读取：**
```java
@RestController
public class ComplexConfigController {
    
    // 读取数组配置
    @Value("${app.servers}")
    private String[] servers;
    
    @Value("${app.ports}")
    private List<Integer> ports;
    
    // 读取Map配置
    @Value("${app.database.host}")
    private String dbHost;
    
    @Value("${app.database.port}")
    private int dbPort;
    
    // 读取嵌套配置
    @Value("${app.redis.pool.max-active}")
    private int redisMaxActive;
    
    @GetMapping("/complex-config")
    public Map<String, Object> getComplexConfig() {
        Map<String, Object> config = new HashMap<>();
        config.put("servers", Arrays.asList(servers));
        config.put("ports", ports);
        config.put("database", Map.of("host", dbHost, "port", dbPort));
        config.put("redisMaxActive", redisMaxActive);
        return config;
    }
}
```

### 2.7.2 Environment接口 - 动态配置读取

**适用场景：** 动态读取配置、条件判断、运行时检查

**基本用法：**
```java
@RestController
public class EnvironmentController {
    
    @Autowired
    private Environment environment;
    
    @GetMapping("/env-config")
    public Map<String, Object> getConfig() {
        Map<String, Object> config = new HashMap<>();
        
        // 读取简单配置
        config.put("port", environment.getProperty("server.port", "8080"));
        config.put("appName", environment.getProperty("spring.application.name", "unknown"));
        
        // 读取数组配置
        String[] servers = environment.getProperty("app.servers", String[].class);
        config.put("servers", servers != null ? Arrays.asList(servers) : Collections.emptyList());
        
        // 读取数字配置
        Integer timeout = environment.getProperty("app.timeout", Integer.class);
        config.put("timeout", timeout);
        
        // 读取布尔配置
        Boolean enabled = environment.getProperty("app.enabled", Boolean.class);
        config.put("enabled", enabled);
        
        return config;
    }
    
    // 检查配置是否存在
    @GetMapping("/check-config")
    public Map<String, Object> checkConfig() {
        Map<String, Object> result = new HashMap<>();
        result.put("hasPort", environment.containsProperty("server.port"));
        result.put("hasAppName", environment.containsProperty("spring.application.name"));
        result.put("hasCustomConfig", environment.containsProperty("app.custom"));
        return result;
    }
    
    // 获取所有配置
    @GetMapping("/all-config")
    public Map<String, Object> getAllConfig() {
        Map<String, Object> result = new HashMap<>();
        
        // 获取所有系统属性
        Properties systemProps = System.getProperties();
        result.put("systemProperties", systemProps);
        
        // 获取所有环境变量
        Map<String, String> envVars = System.getenv();
        result.put("environmentVariables", envVars);
        
        return result;
    }
}
```

**高级用法：**
```java
@Service
public class AdvancedEnvironmentService {
    
    @Autowired
    private Environment environment;
    
    /**
     * 获取配置并指定默认值
     */
    public String getConfigWithDefault(String key, String defaultValue) {
        return environment.getProperty(key, defaultValue);
    }
    
    /**
     * 获取配置并转换为指定类型
     */
    public <T> T getConfigAsType(String key, Class<T> targetType) {
        return environment.getProperty(key, targetType);
    }
    
    /**
     * 获取配置并转换为指定类型，带默认值
     */
    public <T> T getConfigAsType(String key, Class<T> targetType, T defaultValue) {
        return environment.getProperty(key, targetType, defaultValue);
    }
    
    /**
     * 检查配置是否存在
     */
    public boolean hasConfig(String key) {
        return environment.containsProperty(key);
    }
    
    /**
     * 获取所有以指定前缀开头的配置
     */
    public Map<String, String> getConfigsWithPrefix(String prefix) {
        Map<String, String> configs = new HashMap<>();
        
        // 遍历所有配置，找出以指定前缀开头的
        for (String key : environment.getPropertySources().stream()
                .filter(ps -> ps instanceof EnumerablePropertySource)
                .map(ps -> (EnumerablePropertySource<?>) ps)
                .flatMap(ps -> Arrays.stream(ps.getPropertyNames()))
                .collect(Collectors.toSet())) {
            
            if (key.startsWith(prefix)) {
                configs.put(key, environment.getProperty(key));
            }
        }
        
        return configs;
    }
    
    /**
     * 获取配置的原始值（不进行类型转换）
     */
    public String getRawConfig(String key) {
        return environment.getProperty(key);
    }
}
```

### 2.7.3 @ConfigurationProperties - 复杂配置读取

**适用场景：** 复杂配置结构、类型安全、配置验证
```java
// 基础配置类
@ConfigurationProperties(prefix = "app")
@Component
public class AppProperties {
    private String title;
    private String description;
    private int timeout;
    private int retryCount;
    private boolean enabled;
    private boolean debug;
    private List<String> servers;
    private List<Integer> ports;
    private DatabaseConfig database;
    private RedisConfig redis;
    private Map<String, UserConfig> users;
    private List<FeatureConfig> features;
    private Map<String, EnvironmentConfig> environments;
    
    // getter和setter方法
    public String getTitle() { return title; }
    public void setTitle(String title) { this.title = title; }
    
    public String getDescription() { return description; }
    public void setDescription(String description) { this.description = description; }
    
    public int getTimeout() { return timeout; }
    public void setTimeout(int timeout) { this.timeout = timeout; }
    
    public int getRetryCount() { return retryCount; }
    public void setRetryCount(int retryCount) { this.retryCount = retryCount; }
    
    public boolean isEnabled() { return enabled; }
    public void setEnabled(boolean enabled) { this.enabled = enabled; }
    
    public boolean isDebug() { return debug; }
    public void setDebug(boolean debug) { this.debug = debug; }
    
    public List<String> getServers() { return servers; }
    public void setServers(List<String> servers) { this.servers = servers; }
    
    public List<Integer> getPorts() { return ports; }
    public void setPorts(List<Integer> ports) { this.ports = ports; }
    
    public DatabaseConfig getDatabase() { return database; }
    public void setDatabase(DatabaseConfig database) { this.database = database; }
    
    public RedisConfig getRedis() { return redis; }
    public void setRedis(RedisConfig redis) { this.redis = redis; }
    
    public Map<String, UserConfig> getUsers() { return users; }
    public void setUsers(Map<String, UserConfig> users) { this.users = users; }
    
    public List<FeatureConfig> getFeatures() { return features; }
    public void setFeatures(List<FeatureConfig> features) { this.features = features; }
    
    public Map<String, EnvironmentConfig> getEnvironments() { return environments; }
    public void setEnvironments(Map<String, EnvironmentConfig> environments) { this.environments = environments; }
}

// 数据库配置类
public class DatabaseConfig {
    private String host;
    private int port;
    private String name;
    private String username;
    private String password;
    
    // getter和setter方法
    public String getHost() { return host; }
    public void setHost(String host) { this.host = host; }
    
    public int getPort() { return port; }
    public void setPort(int port) { this.port = port; }
    
    public String getName() { return name; }
    public void setName(String name) { this.name = name; }
    
    public String getUsername() { return username; }
    public void setUsername(String username) { this.username = username; }
    
    public String getPassword() { return password; }
    public void setPassword(String password) { this.password = password; }
}

// Redis配置类
public class RedisConfig {
    private String host;
    private int port;
    private int timeout;
    private PoolConfig pool;
    
    // getter和setter方法
    public String getHost() { return host; }
    public void setHost(String host) { this.host = host; }
    
    public int getPort() { return port; }
    public void setPort(int port) { this.port = port; }
    
    public int getTimeout() { return timeout; }
    public void setTimeout(int timeout) { this.timeout = timeout; }
    
    public PoolConfig getPool() { return pool; }
    public void setPool(PoolConfig pool) { this.pool = pool; }
}

// 连接池配置类
public class PoolConfig {
    private int maxActive;
    private int maxIdle;
    private int minIdle;
    
    // getter和setter方法
    public int getMaxActive() { return maxActive; }
    public void setMaxActive(int maxActive) { this.maxActive = maxActive; }
    
    public int getMaxIdle() { return maxIdle; }
    public void setMaxIdle(int maxIdle) { this.maxIdle = maxIdle; }
    
    public int getMinIdle() { return minIdle; }
    public void setMinIdle(int minIdle) { this.minIdle = minIdle; }
}

// 用户配置类
public class UserConfig {
    private String name;
    private String role;
    private List<String> permissions;
    
    // getter和setter方法
    public String getName() { return name; }
    public void setName(String name) { this.name = name; }
    
    public String getRole() { return role; }
    public void setRole(String role) { this.role = role; }
    
    public List<String> getPermissions() { return permissions; }
    public void setPermissions(List<String> permissions) { this.permissions = permissions; }
}

// 功能配置类
public class FeatureConfig {
    private String name;
    private boolean enabled;
    private Map<String, Object> config;
    
    // getter和setter方法
    public String getName() { return name; }
    public void setName(String name) { this.name = name; }
    
    public boolean isEnabled() { return enabled; }
    public void setEnabled(boolean enabled) { this.enabled = enabled; }
    
    public Map<String, Object> getConfig() { return config; }
    public void setConfig(Map<String, Object> config) { this.config = config; }
}

// 环境配置类
public class EnvironmentConfig {
    private boolean debug;
    private String logLevel;
    
    // getter和setter方法
    public boolean isDebug() { return debug; }
    public void setDebug(boolean debug) { this.debug = debug; }
    
    public String getLogLevel() { return logLevel; }
    public void setLogLevel(String logLevel) { this.logLevel = logLevel; }
}
```

### 2.7.4 三种方式对比与选择

| 特性 | @Value | Environment | @ConfigurationProperties |
|------|--------|-------------|-------------------------|
| **适用场景** | 单个配置值 | 动态读取、条件判断 | 复杂配置、类型安全 |
| **类型安全** | 手动转换 | 手动转换 | 自动转换 |
| **性能** | 高（编译时注入） | 中等（运行时获取） | 高（启动时绑定） |
| **验证支持** | 无 | 无 | 支持Bean Validation |
| **IDE支持** | 好 | 一般 | 最好 |
| **重构友好** | 一般 | 差 | 最好 |

**选择建议：**
- **@Value**：简单配置，如端口号、开关等
- **Environment**：需要动态判断或运行时检查的配置
- **@ConfigurationProperties**：复杂配置结构，需要类型安全和验证

### 2.7.5 使用配置的控制器示例
```java
@RestController
@RequestMapping("/api/config")
public class ConfigApiController {
    
    private final AppProperties appProperties;
    
    public ConfigApiController(AppProperties appProperties) {
        this.appProperties = appProperties;
    }
    
    @GetMapping("/basic")
    public Map<String, Object> getBasicConfig() {
        Map<String, Object> config = new HashMap<>();
        config.put("title", appProperties.getTitle());
        config.put("description", appProperties.getDescription());
        config.put("timeout", appProperties.getTimeout());
        config.put("enabled", appProperties.isEnabled());
        return config;
    }
    
    @GetMapping("/database")
    public DatabaseConfig getDatabaseConfig() {
        return appProperties.getDatabase();
    }
    
    @GetMapping("/redis")
    public RedisConfig getRedisConfig() {
        return appProperties.getRedis();
    }
    
    @GetMapping("/users")
    public Map<String, UserConfig> getUsersConfig() {
        return appProperties.getUsers();
    }
    
    @GetMapping("/features")
    public List<FeatureConfig> getFeaturesConfig() {
        return appProperties.getFeatures();
    }
    
    @GetMapping("/environments")
    public Map<String, EnvironmentConfig> getEnvironmentsConfig() {
        return appProperties.getEnvironments();
    }
}
```

## 2.8 复杂数据格式与特殊字符处理

### 2.8.1 Properties格式复杂数据结构

#### 数组和列表
```properties
# 简单数组（逗号分隔）
app.servers=server1,server2,server3
app.ports=8080,8081,8082

# 字符串数组
app.names=张三,李四,王五

# 数字数组
app.weights=1.5,2.0,3.5
```

#### Map结构
```properties
# 简单Map
app.database.host=localhost
app.database.port=3306
app.database.name=mydb

# 嵌套Map
app.redis.host=localhost
app.redis.port=6379
app.redis.pool.max-active=8
app.redis.pool.max-idle=8
app.redis.pool.min-idle=0

# 多层级Map
app.users.admin.name=管理员
app.users.admin.role=ADMIN
app.users.admin.permissions=READ,WRITE,DELETE
app.users.user.name=普通用户
app.users.user.role=USER
app.users.user.permissions=READ
```

#### 特殊字符处理
```properties
# 1. 包含空格的值需要用引号包围
app.message="Hello, World!"
app.title="我的 Spring Boot 应用"

# 2. 包含特殊字符的值需要用引号包围
app.path="C:\\Program Files\\MyApp"
app.url="https://example.com/api?param=value&other=test"
app.regex="^[a-zA-Z0-9]+$"

# 3. 包含等号、冒号等分隔符的值
app.config="key1=value1,key2=value2"
app.time="2024-01-01 12:00:00"
app.json="{\"name\":\"张三\",\"age\":25}"

# 4. 包含Unicode字符
app.unicode="你好世界！\u4e2d\u6587"
app.emoji="Hello \ud83d\ude00 World"

# 5. 包含换行符的值
app.multiline="第一行\n第二行\n第三行"

# 6. 多行值（使用反斜杠续行）
app.description=这是一个很长的描述，\
                可以跨越多行，\
                使用反斜杠连接

# 7. 包含引号的值（需要转义）
app.quoted="He said \"Hello World\""
app.single='He said "Hello World"'

# 8. 包含反斜杠的值（需要转义）
app.windows-path="C:\\\\Users\\\\MyApp\\\\config"
app.regex-escape="\\\\d+"

# 9. 空值和特殊值
app.empty-string=""
app.null-value=null
app.boolean-true=true
app.boolean-false=false

# 10. 包含特殊符号的配置键
app."special-key"=special value
app."key with spaces"=value with spaces
```

### 2.8.2 YAML格式复杂数据结构

#### 数组和列表
```yaml
# 简单数组
app:
  servers:
    - server1
    - server2
    - server3
  ports: [8080, 8081, 8082]  # 行内数组

# 对象数组
  features:
    - name: 用户管理
      enabled: true
      config:
        max-users: 1000
        auto-approve: false
    - name: 数据分析
      enabled: false
      config:
        batch-size: 100
        schedule: "0 0 2 * * ?"

# 混合类型数组
  mixed-list:
    - "字符串"
    - 123
    - true
    - null
    - name: 对象
      value: 测试
```

#### Map和对象结构
```yaml
# 简单Map
app:
  database:
    host: localhost
    port: 3306
    name: mydb
    username: root
    password: 123456

# 嵌套对象
  redis:
    host: localhost
    port: 6379
    timeout: 2000
    pool:
      max-active: 8
      max-idle: 8
      min-idle: 0
      config:
        test-on-borrow: true
        test-while-idle: false

# 复杂嵌套结构
  users:
    admin:
      name: 管理员
      role: ADMIN
      permissions:
        - READ
        - WRITE
        - DELETE
      settings:
        theme: dark
        language: zh-CN
        notifications:
          email: true
          sms: false
    user:
      name: 普通用户
      role: USER
      permissions:
        - READ
      settings:
        theme: light
        language: en-US
        notifications:
          email: true
          sms: true
```

#### 特殊字符和数据类型处理
```yaml
# 1. 多行字符串处理
app:
  # 保持换行符的多行字符串（|）
  description: |
    这是一个多行描述
    可以包含换行符
    保持原始格式
    包括缩进
    
  # 折叠换行符的多行字符串（>）
  sql-query: >
    SELECT * FROM users
    WHERE status = 'active'
    ORDER BY created_at DESC
    
  # 带缩进控制的多行字符串（|- 或 |+）
  code-block: |-
    function hello() {
        console.log("Hello World");
    }
    
  # 折叠字符串（>）
  folded-text: >
    这是一段很长的文本，
    会被折叠成一行，
    但保持可读性。

# 2. 引号使用规则
  quotes:
    # 单引号：不转义特殊字符
    single-quoted: 'Hello "World" with quotes'
    single-with-escape: 'He said ''Hello World'''
    
    # 双引号：转义特殊字符
    double-quoted: "Hello \"World\" with quotes"
    double-with-escape: "He said \"Hello World\""
    
    # 无引号：简单值
    unquoted: Hello World
    unquoted-number: 123
    unquoted-boolean: true

# 3. 特殊字符处理
  special-chars:
    # 包含空格和特殊符号
    message: "Hello, World!"
    title: "我的 Spring Boot 应用"
    
    # 文件路径（Windows和Unix）
    windows-path: "C:\\Program Files\\MyApp"
    unix-path: "/usr/local/myapp"
    
    # URL和查询参数
    url: "https://example.com/api?param=value&other=test"
    encoded-url: "https://example.com/api?param=value%20with%20spaces"
    
    # 正则表达式
    regex: "^[a-zA-Z0-9]+$"
    regex-escape: "\\d+"
    
    # JSON字符串
    json: '{"name":"张三","age":25}'
    json-escaped: "{\"name\":\"张三\",\"age\":25}"

# 4. Unicode和国际化字符
  unicode:
    # 中文字符
    chinese: "你好世界！"
    chinese-unicode: "\u4e2d\u6587"
    
    # Emoji表情
    emoji: "Hello 😀 World"
    emoji-unicode: "Hello \ud83d\ude00 World"
    
    # 其他Unicode字符
    special-symbols: "©®™€£¥"

# 5. 空值和特殊值
  null-values:
    null-value: null
    empty-string: ""
    empty-string-quoted: ""
    zero: 0
    false-value: false

# 6. 布尔值处理
  booleans:
    true-values:
      - true
      - True
      - TRUE
      - on
      - On
      - ON
      - yes
      - Yes
      - YES
    false-values:
      - false
      - False
      - FALSE
      - off
      - Off
      - OFF
      - no
      - No
      - NO

# 7. 数字类型处理
  numbers:
    integer: 123
    negative: -456
    zero: 0
    float: 3.14
    scientific: 1.23e+4
    infinity: .inf
    negative-infinity: -.inf
    not-a-number: .nan

# 8. 日期和时间处理
  datetime:
    date: 2024-01-01
    datetime: 2024-01-01T12:00:00
    datetime-with-timezone: 2024-01-01T12:00:00+08:00
    time: 12:00:00

# 9. 包含特殊字符的键名
  "special-keys":
    "key with spaces": "value with spaces"
    "key-with-dashes": "value with dashes"
    "key_with_underscores": "value with underscores"
    "key.with.dots": "value with dots"
    "key:with:colons": "value with colons"

# 10. 转义字符处理
  escape-chars:
    newline: "Line 1\nLine 2"
    tab: "Column1\tColumn2"
    carriage-return: "Line 1\r\nLine 2"
    backslash: "Path\\to\\file"
    quote-escape: "He said \"Hello\""
    single-quote-escape: 'He said ''Hello'''
```

### 2.8.3 数据格式转换与类型处理

**Properties到YAML转换：** 点号分隔的层级关系转换为YAML的缩进结构

**类型自动转换：** Spring Boot自动进行类型转换
- 数字：`30` → `int`
- 布尔：`true` → `boolean`  
- 小数：`1.5` → `double`
- 数组：`server1,server2` → `List<String>`

### 2.8.4 特殊字符处理示例

**关键处理要点：**
- **引号使用**：包含空格、特殊字符的值必须用引号包围
- **转义处理**：正确转义反斜杠、引号等特殊字符
- **Unicode支持**：使用UTF-8编码，正确处理中文和特殊符号
- **路径处理**：使用跨平台路径分隔符，避免硬编码路径
- **URL编码**：包含特殊字符的URL进行适当编码
- **多行文本**：使用YAML的多行字符串格式处理长文本

### 2.8.5 配置验证

**使用Bean Validation：**
```java
@ConfigurationProperties(prefix = "app")
@Validated
public class AppProperties {
    @NotBlank
    private String title;
    
    @Min(1) @Max(100)
    private int timeout;
    
    @Email
    private String adminEmail;
    
    @Valid
    private DatabaseConfig database;
}
```

**自定义验证器：** 可以创建自定义验证注解和验证器来处理特定业务规则

## 2.9 最佳实践

### 2.9.1 配置组织与管理
- **环境分离**：将不同环境的配置分开（dev、test、prod）
- **敏感信息保护**：使用环境变量或外部配置中心管理敏感信息
- **配置分层**：基础配置、环境特定配置、本地配置分层管理
- **命名规范**：使用小写字母和点号分隔，如 `spring.datasource.url`
- **统一前缀**：自定义配置使用统一前缀，如 `myapp.` 或 `company.`

### 2.9.2 格式选择与结构设计
- **格式选择**：简单配置用Properties，复杂配置用YAML
- **结构清晰**：保持配置结构清晰，避免过深嵌套
- **类型安全**：使用@ConfigurationProperties处理复杂配置
- **验证约束**：为配置添加验证注解，确保配置正确性

### 2.9.3 特殊字符与属性引用
- **引号使用**：包含空格、特殊字符的值必须用引号包围
- **转义处理**：正确转义反斜杠、引号等特殊字符
- **Unicode支持**：使用UTF-8编码，正确处理中文和特殊符号
- **属性引用**：合理使用占位符和默认值，避免循环引用
- **环境变量**：使用环境变量覆盖敏感信息

### 2.9.4 文档化与维护
- **注释说明**：在配置文件中添加注释说明每个配置项的用途
- **团队文档**：为团队维护配置文档，说明配置项的含义和默认值
- **定期审查**：定期审查和清理无用的配置项
- **变更记录**：记录配置变更历史，便于问题排查

## 2.10 总结

Spring Boot的配置系统让应用配置变得简单而强大：

### 核心特性
1. **约定优于配置** - 减少配置工作，提供合理的默认值
2. **多种格式支持** - Properties和YAML，满足不同场景需求
3. **复杂数据结构** - 支持数组、Map、嵌套对象等复杂数据格式
4. **属性引用** - 支持配置值之间的引用和占位符
5. **多种读取方式** - @Value、Environment、@ConfigurationProperties三种方式
6. **类型安全** - 通过注解验证配置，确保配置正确性

### 选择指南

| 场景 | 推荐方式 | 原因 |
|------|----------|------|
| 单个简单配置 | @Value | 简单直接，性能好 |
| 动态读取配置 | Environment | 支持运行时条件判断 |
| 复杂配置结构 | @ConfigurationProperties | 类型安全，支持验证 |

### 关键要点
- **格式选择**：简单配置用Properties，复杂配置用YAML
- **属性引用**：合理使用占位符和默认值，避免循环引用
- **特殊字符**：正确处理引号、转义字符、Unicode等
- **环境管理**：使用Profile分离不同环境配置
- **验证约束**：添加验证注解确保配置正确性

**记住：能用默认配置就用默认配置，需要改变时才写配置！选择合适的方式读取配置，让代码更加清晰和可维护！**